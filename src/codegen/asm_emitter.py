...
class AsmEmitter:
    ...
    def _emit_function_body(self, name: str, instrs: list[IRInstr]) -> None:
        ...
        for instr in instrs:
            ...
            lines = self._emit_instr(instr, alloc, allocator, name, is_leaf)
        ...
        if instrs and instrs[-1].op != IROp.RET:
            ...
            self._output.extend(emit_epilogue(allocator.callee_saved_used, is_leaf))
    ...
    def _emit_instr(self, instr: IRInstr, alloc: dict[str, str],
        ...
        def reg(vreg: str | None) -> str:
            if vreg is None:
                return "R0"
            if vreg.startswith("R") and vreg[1:].isdigit():
                return vreg
            ...
            r = alloc.get(vreg, "R0")
            if r == "__spill":
                return "R0"  # spilled - use R0 as temp (load/store around)
            return r
        ...
        # function calls
            ...
            s1 = reg(instr.src1)
            s2 = reg(instr.src2)
            # pointer arithmetic: multiply index by element size (let's assume 2 for 16-bit, might refine later)
